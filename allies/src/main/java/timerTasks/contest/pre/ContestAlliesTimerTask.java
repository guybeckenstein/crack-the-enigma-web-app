...
public class ContestAlliesTimerTask extends TimerTask {
    private final Gson deserializer;
    private final Type type;
    private final ContestController contestController;

    public ContestAlliesTimerTask(ContestController contestController) {
        super();
        ...
        try {
            ...
            client.newCall(request).enqueue(new Callback() {
                ...
                public void onFailure(@NotNull Call call, @NotNull IOException e) {
                    System.out.println("Failed to get " + contestController.getContestUsername() + "'s Allies.");
                ...
                public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                    try (ResponseBody body = response.body()) {
                        ...
                        Collection<AlliesData> allCurrent = deserializer.fromJson(battlefieldsAsJson.trim(), type);
                        Platform.runLater(() -> contestController.updateContestAlliesTableView(allCurrent));
                    }
                }
            });
